...
public class FallingStarRenderer extends EntityRenderer<FallingEntity> {
    ...
    public static final RenderType STAR_TYPE = withShaderHandler(RenderTypes.createQuadRenderType(StateShards.ADDITIVE_TRANSPARENCY, STAR), ()->{
        ShaderInstance instance = ShaderRegistry.colorGradientTexture.getInstance().get();
    ...
    public static final OBJModel METEOR_TEARDROP = OBJLoader.INSTANCE.loadModel(new OBJModel.ModelSettings(prefix("models/obj/meteor.obj"), false, true, true, true, null));
    public static final SimpleRenderable RENDER = METEOR_TEARDROP.bakeRenderable(StandaloneModelConfiguration.create(Map.of("#qr", prefix("misc/meteor_teardrop"))));
    ...
    public void render(FallingEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        ...
        float beamWidth = 5f;
        float starSize = 3f;
        RENDER.render(poseStack, bufferSource, RenderType::entitySolid, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY, partialTicks, MultipartTransforms.EMPTY);
