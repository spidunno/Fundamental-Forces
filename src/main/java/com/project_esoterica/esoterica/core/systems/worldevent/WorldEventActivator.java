        ...
        PlayerDataCapability.getCapability(player).ifPresent(capability -> {
            if (areStarfallsAllowed(level)) {
                if (!capability.firstTimeJoin) {
                    WorldEventManager.addWorldEvent(level, new StarfallEvent(StarfallActors.INITIAL_SPACE_DEBRIS).targetEntity(player).randomizedStartingCountdown(level).looping().determined());
                } else {
