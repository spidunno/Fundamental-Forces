...
import team.lodestar.fufo.common.item.FluidTankItem;
import team.lodestar.fufo.common.logistics.fluid_tank.FluidTankBlock;
import team.lodestar.fufo.common.logistics.fluid_tank.FluidTankBlockEntity;
import team.lodestar.fufo.common.logistics.sealed_barrel.SealedBarrelBlock;
import team.lodestar.fufo.core.setup.content.block.BlockPropertiesRegistry;
...
public class BlockRegistrate {
    ...
    public static final BlockEntry<PipeNodeBlock<PipeNodeBlockEntity>> PIPE_ANCHOR = setupItemBlock("anchor",
        (p) -> new PipeNodeBlock<>(p).<PipeNodeBlock<PipeNodeBlockEntity>>setBlockEntity(BlockEntityRegistrate.ANCHOR), BlockPropertiesRegistry.CRUDE_PROPERTIES())
        .blockstate(predefinedState())
        .register();

    public static final BlockEntry<FluidTankBlock<FluidTankBlockEntity>> FLUID_TANK = setupBlock("fluid_tank", (p) -> new FluidTankBlock<>(p).<FluidTankBlock<FluidTankBlockEntity>>setBlockEntity(BlockEntityRegistrate.FLUID_TANK), BlockPropertiesRegistry.CRUDE_PROPERTIES())
        .blockstate((ctx, p) -> {
    ...
    public static final BlockEntry<SealedBarrelBlock<SealedBarrelBlockEntity>> SEALED_BARREL = setupBlock("sealed_barrel", (p) -> new SealedBarrelBlock<>(p).<SealedBarrelBlock<SealedBarrelBlockEntity>>setBlockEntity(BlockEntityRegistrate.SEALED_BARREL), BlockPropertiesRegistry.CRUDE_PROPERTIES())
        .blockstate((ctx, p) -> {
    ...
    public static final BlockEntry<PipeNodeBlock<ValveBlockEntity>> VALVE = setupBlock("valve",
        (p) -> new PipeNodeBlock<ValveBlockEntity>(p).<PipeNodeBlock<ValveBlockEntity>>setBlockEntity(BlockEntityRegistrate.VALVE), BlockPropertiesRegistry.CRUDE_PROPERTIES())
        .blockstate((ctx, p) -> {
    ...
    public static final BlockEntry<PipeNodeBlock<PumpBlockEntity>> PUMP = setupBlock("pump",
        (p) -> new PipeNodeBlock<PumpBlockEntity>(p).<PipeNodeBlock<PumpBlockEntity>>setBlockEntity(BlockEntityRegistrate.PUMP), BlockPropertiesRegistry.CRUDE_PROPERTIES())
...
//        .register();
    ...
    public static final BlockEntry<ArrayBlock<ArrayBlockEntity>> CRUDE_ARRAY = setupItemBlock("crude_array", (p) -> new ArrayBlock<>(p).<ArrayBlock<ArrayBlockEntity>>setBlockEntity(BlockEntityRegistrate.CRUDE_ARRAY), BlockPropertiesRegistry.CRUDE_PROPERTIES())
        .blockstate((ctx, p) -> p.horizontalBlock(ctx.get(), p.models().getExistingFile(fufoPath("block/crude_array"))))
        .register();
    public static final BlockEntry<CrudePrimerBlock<CrudePrimerBlockEntity>> CRUDE_PRIMER = setupItemBlock("crude_primer", (p) -> new CrudePrimerBlock<>(p).<CrudePrimerBlock<CrudePrimerBlockEntity>>setBlockEntity(BlockEntityRegistrate.CRUDE_PRIMER), BlockPropertiesRegistry.CRUDE_PROPERTIES())
        .blockstate((ctx, p) -> p.horizontalBlock(ctx.get(), p.models().getExistingFile(fufoPath("block/crude_primer"))))
        .register();
    public static final BlockEntry<CrudeNeedleBlock<CrudeNeedleBlockEntity>> CRUDE_NEEDLE = setupItemBlock("crude_needle", (p) -> new CrudeNeedleBlock<>(p).<CrudeNeedleBlock<CrudeNeedleBlockEntity>>setBlockEntity(BlockEntityRegistrate.CRUDE_NEEDLE), BlockPropertiesRegistry.CRUDE_PROPERTIES()).blockstate(invisibleState()).register();
    public static final BlockEntry<UITestBlock<UITestBlockEntity>> UI_TEST = setupItemBlock("ui_test", (p) -> new UITestBlock<>(p).<UITestBlock<UITestBlockEntity>>setBlockEntity(BlockEntityRegistrate.UI_TEST_BLOCK), BlockPropertiesRegistry.CRUDE_PROPERTIES()).blockstate(invisibleState()).register();

    public static final BlockEntry<MeteorFlameBlock<MeteorFlameBlockEntity>> METEOR_FIRE = setupBlock("meteor_fire", (p) -> new MeteorFlameBlock<>(p).<MeteorFlameBlock<MeteorFlameBlockEntity>>setBlockEntity(BlockEntityRegistrate.METEOR_FLAME), BlockPropertiesRegistry.METEOR_FIRE_PROPERTIES()).blockstate(predefinedState()).register();
    public static final BlockEntry<FlammableMeteoriteBlock> ORTUSITE = setupItemBlock("ortusite", (p) -> new FlammableMeteoriteBlock(p, (s, b) -> METEOR_FIRE.getDefaultState()), BlockPropertiesRegistry.ASTEROID_PROPERTIES())
    ...
    public static final BlockEntry<OrbBlock<OrbBlockEntity>> FORCE_ORB = setupBlock("force_orb", (p) -> new OrbBlock<>(p).<OrbBlock<OrbBlockEntity>>setBlockEntity(BlockEntityRegistrate.ORB), BlockPropertiesRegistry.ORB_PROPERTIES()).blockstate(invisibleState()).register();

    public static final BlockEntry<Block> BLOCK_OF_CRACK = setupItemBlock("block_of_crack", Block::new, BlockPropertiesRegistry.CRACK_PROPERTIES()).register();
